...
const addClassTech = (event) => {
  const addTechElement = document.querySelector('.tech');
  addTechElement.classList.remove('tech');
...
input.addEventListener('input', (event) => {
  const addTechElement = document.querySelector('.tech');
  addTechElement.innerText = event.target.value;
});
...
myWebpage.addEventListener('dblclick', () => {
  window.location.replace('https://larissamaehara.github.io/');
});
