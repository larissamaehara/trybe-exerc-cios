...
const findPersonByName = (name) => {
  let person
  // Bloco de try…catch faz uma validação se a variável person continua ou não como undefined
  try {
    ...
    const receiver = `Destinatário: ${person.name}`
    ...
    return `${receiver}. ${fullAddress}. ${cep}`
  } catch (error) {
...
const arrayValidation = (array) => {
  if (array.length === 0) {
    throw new Error('Nenhuma pessoa mora nesse estado, tente outro')
  }
}
